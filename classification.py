...
def classifyKNN(metrics, classes): 
    """Creates a K Nearest Neighbors algorithm trained on the data in metrics and classes. 
    Metrics and classes are lists of the same length. Returns the fitted function."""
    n_neighbors = 15
    clf = neighbors.KNeighborsClassifier(n_neighbors)
    clf.fit(metrics, classes)
    return clf 
    
def classify(metrics, classes):
    """Creates a Support Vector Machine trained on the data in metrics and classes. 
    Metrics and classes are lists of the same length. Returns the fitted function."""
    weights = {}
    weights[0] = 0.1 
    weights[1] = 2.5
